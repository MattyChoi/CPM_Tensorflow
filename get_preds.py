...
tf.app.flags.DEFINE_string('DEMO_TYPE',
                           ...
                           default_value=r'C:\Users\matth\OneDrive\Documents\Storage\CSCI_5561\cpm-tf\test_imgs\test_0000035.jpg',
...
def main(argv):
    ...
    # iamge processing
    with tf.device(tf_device):
        val_path = os.path.join(path, 'val', 'val_annotation.json')
        train_path = os.path.join(path, 'utils', 'dataset', 'train_annotation.json')

        with open(val_path) as f:
            json_dic = json.load(f)

        for i in range(len(json_dic['data'])):
            info_slice = json_dic['data'][i]
            test_img_t = time.time()

            # read the image here
            val_img_path = os.path.join(path, 'val')
            train_img_path = os.path.join(path, 'utils', 'dataset', 'train')

            img_path = os.path.join(val_img_path, info_slice['file'])
            # test_img = cpm_utils.read_image(img_path, [], FLAGS.input_size, 'IMAGE')
            ...
            last_heatmap = stage_heatmap_np[-1][0, :, :, 0:flags.joints].reshape((flags.hmap_size, flags.hmap_size, flags.joints))
            last_heatmap = cv2.resize(last_heatmap, (test_img.shape[1], test_img.shape[0]))
            # print(last_heatmap.shape)

            landmarks = []
            for joint_num in range(flags.joints):
                y, x = np.unravel_index(np.argmax(last_heatmap[:, :, joint_num]),
                                            (test_img.shape[0], test_img.shape[1]))
                landmarks.append(x.astype(int))
                landmarks.append(y.astype(int))
            
            if (i) % 100 == 0:
                print(len(landmarks))
                print(info_slice['file'])
                print(info_slice['landmarks'])
                print(landmarks)
                print('------------------------------------------------------')
                print('fps: %.2f' % (1 / (time.time() - fps_t)))

            json_dic['data'][i]['landmarks'] = landmarks
        
        # dump into json file
        json_string = json.dumps(json_dic)
        with open('val_preds.json', 'w') as outfile:
