...
def main(argv):
    ...
    test_log_save_dir = os.path.join('models',
                                     ...
                                     model_path_suffix,
                                     'test')
    os.system('mkdir {}'.format(model_save_dir))
    os.system('mkdir {}'.format(train_log_save_dir))
    os.system('mkdir {}'.format(test_log_save_dir))
    ...
    #                                                          FLAGS.batch_size, FLAGS.input_size, True, True,
    #                                                          FLAGS.augmentation_config, FLAGS.hnm, FLAGS.do_cropping)
    g = Ensemble_data_generator.ensemble_data_generator("omc_dataset_train.tfrecords", FLAGS.batch_size, 256, 256)
    # g_eval = Ensemble_data_generator.ensemble_data_generator("omc_dataset_val.tfrecords", 5, 256, 256)

    """ Build network graph
    """
    model = cpm_model.CPM_Model(stages=FLAGS.cpm_stages,
    ...
    with tf.Session(config=tf.ConfigProto(device_count=device_count,
        ...
        for training_itr in range(FLAGS.training_iters):
            ...
            #     while cnt < 10:
            #         batch_x_np, batch_joints_np = sess.run(next_ele_val)
            #         # Normalize images
            #         batch_x_np = batch_x_np / 255.0 - 0.5
